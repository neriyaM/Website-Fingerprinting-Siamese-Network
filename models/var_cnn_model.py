...
def var_cnn(input_shape, emb_size):
    dir_input = Input(shape=input_shape, name='dir_input')
    ...
    metadata_output = Dense(32)(metadata_input)  # consider this the embedding of all the metadata
    metadata_output = BatchNormalization()(metadata_output)
    metadata_output = Activation('relu')(metadata_output)
    input_params = [dir_input, time_input, metadata_input]
    combined = Concatenate()([dir_output, time_output, metadata_output])
    model_output = Dense(emb_size, name='model_output')(combined)
